/* Formazin Energie-Funnel — Vollbild, nicht scrollbar. Schreibt direkt nach Supabase (anon, RLS). */
(function () {
  "use strict";
  ...
  var GEBAEUDE = [
    ...
    { k: "gewerbe_nwg", t: "Gewerbe / NWG" }, { k: "denkmal", t: "Denkmal / Bestand" }
  ];
  ...
  /* ---- Bausteine ---- */
  function progress(showBack) {
    var s = stepIdx(), pct = (s / STEPS.length) * 100;
    return '<div class="f-toprow">' +
  ...
  function head(showBack, title, sub) {
    return progress(showBack) + '<h2 class="f-title">' + title + '</h2>' + (sub ? '<p class="f-sub">' + sub + '</p>' : '');
  }
  ...
  function shell(headHTML, bodyHTML, footHTML) {
    return '<div class="f-head">' + headHTML + '</div><div class="f-body">' + bodyHTML + '</div>' +
      (footHTML ? '<div class="f-foot">' + footHTML + '</div>' : '');
  }
  function center(html) { return '<div class="f-center">' + html + '</div>'; }

  function render() {
    var h = "";
    if (screen === "anliegen") {
      h = shell(head(false, "Worum geht es bei Ihnen?", "Wählen Sie Ihr Anliegen — wir führen Sie in wenigen Schritten durch."),
        '<div class="f-grid">' + ANLIEGEN.map(function (c) { return choice(c, false, "anliegen"); }).join("") + "</div>");
    } else if (screen === "prequal") {
      h = center('<button class="f-back" data-act="back" style="position:absolute;top:18px;left:18px">' + I.arrowL + ' Zurück</button>' +
        '<h2 class="f-title">Kurz vorweg — ehrlich gesagt</h2>' +
        '<p class="f-sub" style="margin-top:12px;max-width:42ch">Die Heizungsanlage selbst plant und baut Ihr <b>Heizungs-Fachbetrieb</b>. Wir sind Ihr Partner für die <b style="color:var(--primary)">Förderung</b> dazu: Wir holen für Ihren Heizungstausch das Maximum an Fördermitteln heraus und übernehmen Anträge und Nachweise.</p>' +
        '<div style="display:grid;gap:10px;margin-top:24px;width:100%;max-width:360px">' +
        '<button class="btn f-full" data-act="prequal-yes">Ja, zur Förderung beraten ' + I.arrowR + '</button>' +
        '<button class="choice" data-act="prequal-no" style="justify-content:center"><span class="ct" style="text-align:center"><b>Nein, nur die Anlage selbst</b></span></button></div>');
    } else if (screen === "exit") {
      h = center('<div class="ok-icon">' + I.help + '</div>' +
        '<h2 class="f-title">Dann ist Ihr Fachbetrieb die richtige Adresse</h2>' +
        '<p class="f-sub" style="margin-top:12px;max-width:44ch">Für die reine Heizungsplanung und den Einbau wenden Sie sich am besten direkt an einen Heizungs-Fachbetrieb. Sobald es um <b>Förderung</b>, einen <b>Energieausweis</b> oder einen <b>Sanierungsfahrplan</b> geht, sind wir gern für Sie da.</p>' +
        '<button class="btn" data-act="reset" style="margin-top:22px">Zurück zum Anfang</button>');
    } else if (screen === "gebaeude") {
      var body = '<div class="f-grid two">' + GEBAEUDE.map(function (c) { return choice(c, A.gebaeudetyp === c.k, "geb"); }).join("") + "</div>" +
        '<p class="f-label">Baujahr</p><div class="f-pills">' +
        BAUJAHR.map(function (b) { return '<button class="pill' + (A.baujahr_spanne === b ? ' sel' : '') + '" data-act="bj" data-k="' + b + '">' + b + "</button>"; }).join("") + "</div>" +
        '<div class="f-grid two" style="margin-top:14px"><input class="field" data-f="plz" inputmode="numeric" maxlength="5" placeholder="PLZ" value="' + (A.plz || "") + '"><input class="field" data-f="ort" placeholder="Ort" value="' + (A.ort || "") + '"></div>' +
        (A.im_servicegebiet === false ? '<p class="f-sub" style="margin-top:8px;font-size:.86rem">Hinweis: Wir arbeiten vor allem in Berlin &amp; Brandenburg. Schreiben Sie uns trotzdem — wir prüfen, ob wir Ihr Vorhaben übernehmen können.</p>' : "");
      h = shell(head(true, "Um welches Gebäude geht es?"), body,
        '<button class="btn f-full' + (A.gebaeudetyp ? "" : " disabled") + '" data-act="geb-next">Weiter ' + I.arrowR + "</button>");
    } else if (screen === "kontext") {
      var kx = KONTEXT[A.intent];
      h = shell(head(true, kx ? kx.q : "Erzählen Sie uns mehr"),
        '<div class="f-grid two">' + (kx ? kx.o.map(function (c) { return choice(c, A.massnahme === c.t, "ctx"); }).join("") : choice({ k: "weiter", t: "Weiter" }, false, "ctx-skip")) + "</div>");
    } else if (screen === "zeitrahmen") {
      h = shell(head(true, "Wie konkret ist Ihr Vorhaben?"),
        '<div class="f-grid">' + ZEIT.map(function (c) { return choice(c, A.zeitrahmen === c.k, "zeit"); }).join("") + "</div>");
    } else if (screen === "kontakt") {
      var body2 = '<div class="f-grid">' +
        ...
        '<textarea class="field" data-f="nachricht" rows="2" placeholder="Ihre Nachricht (optional)">' + (A.nachricht || "") + '</textarea>' +
      ...
      h = shell(head(true, "Wohin dürfen wir uns melden?", "Wir melden uns innerhalb von 1–2 Werktagen mit einer konkreten Einschätzung."), body2,
        '<button class="btn f-full' + (kontaktOk() ? "" : " disabled") + '" data-act="to-summary">Anfrage prüfen ' + I.arrowR + "</button>");
      ...
      var bodyS = '<div class="summary-list">' + rows.filter(function (r) { return r[1]; }).map(function (r) { return '<div class="r"><span class="k">' + r[0] + '</span><span class="v">' + esc(r[1]) + "</span></div>"; }).join("") + "</div>" +
        (err ? '<div class="err">' + esc(err) + "</div>" : "");
      h = shell('<div class="f-toprow"><button class="f-back" data-act="back">' + I.arrowL + ' Zurück</button><span class="f-step">Übersicht</span></div><h2 class="f-title">Passt das so?</h2>', bodyS,
        '<button class="btn f-full' + (busy ? " disabled" : "") + '" data-act="submit">' + (busy ? "Wird gesendet…" : "Anfrage absenden " + I.check) + "</button>");
    } else if (screen === "success") {
      h = center('<div class="ok-icon">' + I.check + '</div><h2 class="f-title">Vielen Dank' + (A.name ? ", " + esc(A.name.split(" ")[0]) : "") + '!</h2>' +
        '<p class="f-sub" style="margin-top:12px;max-width:44ch">Ihre Anfrage liegt bei uns. Wir melden uns innerhalb von <b>1–2 Werktagen</b> mit einer konkreten Einschätzung — telefonisch oder per E-Mail.</p>' +
        '<a class="btn" href="index.html" style="margin-top:22px">Zurück zur Übersicht</a>');
    }
    el.innerHTML = h;
    var b = el.querySelector(".f-body"); if (b) b.scrollTop = 0;
  }
  ...
  function onInput(e) {
    ...
    A[f] = e.target.value;
    if (f === "plz") {
      var clean = e.target.value.replace(/\D/g, "").slice(0, 5);
      var changed = A.plz !== clean || (clean.length === 5 && A.im_servicegebiet === undefined);
      A.plz = clean; A.im_servicegebiet = clean.length === 5 ? servicegebiet(clean) : undefined;
      if (changed) { render(); var pe = el.querySelector('[data-f="plz"]'); if (pe) { pe.focus(); pe.setSelectionRange(clean.length, clean.length); } return; }
    }
    if (screen === "kontakt") toggleBtn();
  }
  function toggleBtn() {
    var btn = el.querySelector('[data-act="to-summary"]'); if (btn) btn.classList.toggle("disabled", !kontaktOk());
  }
